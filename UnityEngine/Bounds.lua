...
local zero = Vector3.retain()

local Bounds = 
{
	center = Vector3.retain(),
	extents = Vector3.retain(),
}

local get = tolua and tolua.initget(Bounds) or {}
...
function Bounds:Expand(amount)	
	if type(amount) == "number" then
		amount = amount * 0.5
		self.extents:Add(Vector3.static(amount, amount, amount))
	else

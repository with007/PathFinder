...
local _forward = Vector3.retain(0, 0, 1)
local _up = Vector3.retain(0, 1, 0)
local _next = { 2, 3, 1 }

local Quaternion = {}
local get = tolua and tolua.initget(Quaternion) or {}
...
local CONST_ZERO = Quaternion.create(0, 0, 0, 0)
local CONST_ONE = Quaternion.create(1, 1, 1, 1)
local STATIC = Quaternion.create(0, 0, 0, 0)

function Quaternion.constZero()
    return CONST_ZERO
end

function Quaternion.constOne()
    return CONST_ONE
end

function Quaternion.static(x, y, z, w)
    STATIC:init(x, y, z, w)
    return STATIC
end

function Quaternion.new(x, y, z, w)
	return ClsFactory.getClsAutoRecycle(ClsFactory.ClassName.Quaternion, x, y, z, w)
end

-- for LuaWrapper
Quaternion.New = Quaternion.new

function Quaternion.retain(x, y, z, w)
    return ClsFactory.getCls(ClsFactory.ClassName.Quaternion, x, y, z, w)
end

function Quaternion.release(q)
    ClsFactory.addCls(q)
end
...
function Quaternion.Euler(x, y, z)
	...
	return _new(y * sinX * z + sinY * x * sinZ, sinY * x * z - y * sinX * sinZ, y * x * sinZ - sinY * sinX * z, y * x * z + sinY * sinX * sinZ)
end
...
function Quaternion:SetFromToRotation1(from, to)
	...
	elseif d > 1 - 1e-6 then
		...
		local axis = Vector3.Cross(Vector3.static(1, 0, 0), v0)
		
		if axis:SqrMagnitude() < 1e-6 then
			axis = Vector3.Cross(Vector3.static(0, 0, 1), v0)
		end
...
local function UnclampedSlerp(q1, q2, t)		
    ...
    if dot < 0.95 then		
		...
		return _new(q1.x * t1 + q2.x * t2, q1.y * t1 + q2.y * t2, q1.z * t1 + q2.z * t2, q1.w * t1 + q2.w * t2)
	else
		q1 = _new(q1.x + t * (q2.x - q1.x), q1.y + t * (q2.y - q1.y), q1.z + t * (q2.z - q1.z), q1.w + t * (q2.w - q1.w))
...
local function SanitizeEuler_S(eulerx, eulery, eulerz)
	if eulerx < negativeFlip then
		eulerx = eulerx + two_pi
	elseif eulerx > positiveFlip then
		eulerx = eulerx - two_pi
	end

	if eulery < negativeFlip then
		eulery = eulery + two_pi
	elseif eulery > positiveFlip then
		eulery = eulery - two_pi
	end

	if eulerz < negativeFlip then
		eulerz = eulerz + two_pi
	elseif eulerz > positiveFlip then
		eulerz = eulerz + two_pi
	end
	return eulerx, eulery, eulerz
end

function Quaternion_S.AngleAxis(angle, axisx, axisy, axisz)
	local normAxisX, normAxisY, normAxisZ = Vector3_S.Normalize(axisx, axisy, axisz)
    angle = angle * halfDegToRad
    local s = sin(angle)
...
function Quaternion_S.Normalize(selfx, selfy, selfz, selfw)
	local n = selfx * selfx + selfy * selfy + selfz * selfz + selfw * selfw
	...
	return selfx, selfy, selfz, selfw
end

function Quaternion_S.LookRotation(forwardx, forwardy, forwardz, upx, upy, upz)
	local x, y, z, w = Quaternion.const_identity:Get()
	local mag = Vector3_S.Magnitude(forwardx, forwardy, forwardz)
	if mag < 1e-6 then
		return x, y, z, w
	end
	
	forwardx, forwardy, forwardz = Vector3_S.Div(forwardx, forwardy, forwardz, mag)
	upx, upy, upz = upx or _up.x, upy or _up.y, upz or _up.z
	local rightx, righty, rightz = Vector3_S.Cross(upx, upy, upz, forwardx, forwardy, forwardz)
	local rightxn, rightyn, rightzn = Vector3_S.Normalize(rightx, righty, rightz)
	upx, upy, upz = Vector3_S.Cross(forwardx, forwardy, forwardz, rightxn, rightyn, rightzn)

	local t = rightx + upy + forwardz
	...
	if t > 0 then		
		...
		local s = 0.5 / sqrt(t)
		q[i] = s * t
		local w2 = (rot[k][j] - rot[j][k]) * s
		q[j] = (rot[j][i] + rot[i][j]) * s
		q[k] = (rot[k][i] + rot[i][k]) * s
		
		x, y, z, w = q[1], q[2], q[3], w2
	end

	return Quaternion_S.Normalize(x, y, z, w)
end

function Quaternion_S.ToEulerAngles(x, y, z, w)
	local retX, retY, retZ = Vector3_S.Zero()
	local check = 2 * (y * z - w * x)
	
	if check < 0.999 then
		if check > -0.999 then
			retX, retY, retZ = -asin(check), atan2(2 * (x * z + w * y), 1 - 2 * (x * x + y * y)), atan2(2 * (x * y + w * z), 1 - 2 * (x * x + z * z))
			retX, retY, retZ = SanitizeEuler_S(retX, retY, retZ)
			return Vector3_S.Mul(retX, retY, retZ, rad2Deg)
		else
			retX, retY, retZ = half_pi, atan2(2 * (x * y - w * z), 1 - 2 * (y * y + z * z)), 0
			retX, retY, retZ = SanitizeEuler_S(retX, retY, retZ)
			return Vector3_S.Mul(retX, retY, retZ, rad2Deg)
		end
	else
...
function Quaternion_S.MulVec3(x, y, z, w, pointx, pointy, pointz)
	local num 	= x * 2
	local num2 	= y * 2
	local num3 	= z * 2
	local num4 	= x * num
	...
	local retx = (((1 - (num5 + num6)) * pointx) + ((num7 - num12) * pointy)) + ((num8 + num11) * pointz)
	local rety = (((num7 + num12) * pointx) + ((1 - (num4 + num6)) * pointy)) + ((num9 - num10) * pointz)
	local retz = (((num8 - num11) * pointx) + ((num9 + num10) * pointy)) + ((1 - (num4 + num5)) * pointz)
